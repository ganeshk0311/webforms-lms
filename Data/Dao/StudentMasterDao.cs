...
namespace FlmWebFormsApp.Data.Dao
{
    public class StudentMasterDao : LmsDao
    {
        ...
        public override int PerformInsertUpdateDelete(dynamic model, string operation)
        {
            ...
            {
                query = "insert into studentmaster (studname, emailid, password, gender, state, city, " +
                        "mobile, address, dob, photo, fkcourseid) " +
                        "values('"+student.StudName+"','"+student.Emailid+"','"+student.Password+"'," +
                        "'"+student.Gender+"','"+student.State+"','"+student.City+"','"+student.Mobile+"'," +
                        "'"+student.Address+"','"+student.Dob+"','"+student.Photo+"','"+student.FkCourseId+"')";                
            }
            ...
            {
                query = "delete from studentmaster where id=" + student.Id;
            }
            else {
                return 0;
            }
        ...
        {
            string query = "select * from studentmaster where emailid='" + student.Emailid + "' " +
