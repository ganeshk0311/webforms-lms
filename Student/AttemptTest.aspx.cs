    ...
    public partial class AttemptTest : System.Web.UI.Page
    {
        ...
        protected void BtnContinue_Click(object sender, EventArgs e)
        {
            ...
            else 
            {
                ...
                StudentAttemptSummary attemptSummary = new StudentAttemptSummary();
                attemptSummary.FkStudId = Convert.ToInt32(Session["studid"]);
                attemptSummary.FkTestId = Convert.ToInt32(Session["testid"]);
                attemptSummary.AttemptDate = DateTime.Now;
                if (correctAnswersCount >= (dtQuestions.Rows.Count / 2))
                {
                    attemptSummary.Result = true;
                }
                else 
                { 
                    attemptSummary.Result = false;
                }
                StudentAttemptSummaryDao summaryDao = new StudentAttemptSummaryDao();
                summaryDao.PerformInsertUpdateDelete(attemptSummary, "save");
